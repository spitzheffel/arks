...
- [-] 18. 后端 - 缺口检测
  ...
  - [x] 18.4 实现批量缺口检测接口
  - [x] 18.5 实现定时缺口检测
  - [ ] 18.6 缺口检测仅处理符合条件的交易对

- [ ] 19. 后端 - 缺口回补
  - [ ] 19.1 实现单个缺口回补
  - [ ] 19.2 实现批量缺口回补 (带限流)
  - [ ] 19.3 实现自动缺口回补 (检查全局和周期级开关)
  - [ ] 19.4 实现回补状态流转
  - [ ] 19.5 实现回补重试机制
  - [ ] 19.6 缺口回补创建 sync_task 记录
  - [ ] 19.7 缺口回补成功后更新 sync_status
...
- [ ] 21. 后端 - 配置管理
  - [ ] 21.1 实现 SystemConfig 实体和 Mapper
  - [ ] 21.2 实现 ConfigService
  - [ ] 21.3 实现 ConfigController
...
- [ ] 27. 二次确认弹窗验收
  - [ ] 27.1 验证数据源删除/启用/禁用二次确认
  - [ ] 27.2 验证市场启用/禁用二次确认
  - [ ] 27.3 验证交易对同步开关二次确认
  - [ ] 27.4 验证删除历史数据二次确认

- [ ] 28. 自动化测试验收
  - [ ] 28.1 验证后端单测通过 (mvn -q test)
  - [ ] 28.2 验证前端 lint/typecheck/test/test:e2e 通过
  - [ ] 28.3 验证关键流程 E2E 冒烟通过
  - [ ] 28.4 验证 E2E Mock 开关仅覆盖外部 API

## 自动化测试

- [ ] 29. 后端 - 单测与集成测试
  - [ ] 29.1 覆盖历史同步增量策略与时间校验
  - [ ] 29.2 覆盖缺口检测与回补流程
  - [ ] 29.3 覆盖加密存储与代理配置

- [ ] 30. 前端 - 组件测试
  - [ ] 30.1 覆盖数据同步页面时间范围输入与校验
  - [ ] 30.2 覆盖自动回补开关配置入口

- [ ] 31. E2E 冒烟 (Playwright/Chromium)
  - [ ] 31.1 E2E 环境准备 (EXCHANGE_API_MOCK=true)
  - [ ] 31.2 数据源新增 + 连接测试
  - [ ] 31.3 手动历史同步 (指定时间范围)
  - [ ] 31.4 缺口检测 + 回补流程
