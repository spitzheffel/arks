...
- 4.5 用户可以手动触发历史数据同步（必须指定 startTime/endTime）
- 4.6 系统支持定时自动同步（可配置同步频率）
- 4.7 用户可以查看同步任务状态和历史记录
- 4.8 系统记录每个交易对每个周期的最后同步时间、最后K线时间与总K线数量（无数据时 `last_kline_time` 为 NULL）
- 4.9 同步失败时系统记录错误信息并支持重试
- 4.10 只有开启了数据同步的交易对才会被同步
- 4.11 定时/自动同步仅处理启用的数据源、启用的市场以及已开启同步的交易对
- 4.12 自动同步仅处理交易对配置的同步周期
- 4.13 定时增量同步基于 `last_kline_time` 追赶；首次启用或 `last_kline_time` 为 NULL 时仅补前一日数据，避免自动全量拉取
- 4.14 停机导致跨度过大时按时间范围分段追赶（单次跨度不超过30天）
- 4.15 所有定时任务按 UTC 时区执行
- 4.16 最小同步周期为 1m，不启用 1s
- 4.17 实时同步全局开关关闭时，立即断开已有 WebSocket 订阅并停止新订阅
...
- 5.8 系统支持自动缺口回补（可配置：全局开关 + 交易对+周期级别开关；手动删除历史数据后该周期自动回补默认关闭，需手动开启）
...
- 自动化测试为硬性要求：后端单测、前端单测与 E2E 冒烟必须通过
- 自动化测试未通过禁止交付
